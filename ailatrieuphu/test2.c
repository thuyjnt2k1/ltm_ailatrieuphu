    ...
    while (1)
    {
        if (flag == 1)
        {
            ...
            else
            {
                ...
                ZeroMemory(buffer, MAX_BUFF);
                sprintf(buffer, "result:%s %d", idRoom, count);
                send(sockfd, buffer, strlen(buffer), 0);
                ...
                {
                    ZeroMemory(buffer, MAX_BUFF);
                    sprintf(buffer, "getResult:%s %s", idRoom, username);
                    send(sockfd, buffer, strlen(buffer), 0);
                    printf("getrESULT :%s\n", buffer);
                    delay(1);
                    ZeroMemory(buffer, MAX_BUFF);
                    nread = recv(sockfd, buffer, MAX_BUFF, 0);
                    printf("recv: %s \n", buffer);

                    char *c = strtok(buffer, ":");
                    if (strcmp(c, "notend") == 0)
                    {
                        continue;
                    }
                    else
